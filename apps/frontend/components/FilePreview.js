...
import {
    CloseOutlineIcon,          // ✅ ProfileImageUpload에서 사용 중
    ErrorCircleIcon,           // ✅ 여러 곳에서 사용 중
    ImageIcon,
    PdfIcon,
    MovieIcon,
    RefreshOutlineIcon,        // LoadingOutlineIcon 대신 (chat/index.js에서 사용)
    MusicIcon,
    AttachFileOutlineIcon      // FileIcon 대신 (ChatInput.js에서 사용)
} from '@vapor-ui/icons';
...
const FilePreview = ({
    ...
    const processFile = useCallback(async (file) => {
        try {
            const validationResult = fileService.validateFile(file);
    ...
    useEffect(() => {
        ...
        const handlePaste = async (e) => {
            ...
            const fileItems = Array.from(items).filter(
                item => item.kind === 'file' &&
                    (item.type === 'image/jpeg' || item.type === 'application/pdf')
            );
    ...
    const getFileIcon = useCallback((file) => {
        ...
        if (file.type.startsWith('image/')) {
            ...
            return <MovieIcon {...iconProps} color="#2196F3" />;
        } else if (file.type.startsWith('audio/')) {
            return <MusicIcon {...iconProps} color="#9C27B0" />;
        } else if (file.type === 'application/pdf') {
            return <PdfIcon {...iconProps} color="#F44336" />;
        } else {
            return <AttachFileOutlineIcon {...iconProps} color="#757575" />;
        }
    }, [variant]);
        ...
        if (uploadError) {
            return (
                <Callout color="danger" className="mt-2 flex items-center gap-2">
                    <ErrorCircleIcon className="w-4 h-4" aria-hidden="true" />
        ...
        if (uploading) {
            return (
                <div className="mt-2 flex items-center gap-2 text-sm text-gray-600">
                    <RefreshOutlineIcon className="w-4 h-4 animate-spin" aria-hidden="true" />
                            ...
                            {variant !== 'readonly' && (
                                <div className="flex-shrink-0">
                                    {!uploading && onRemove && (
                                        ...
                                        >
                                            <CloseOutlineIcon className="w-4 h-4" aria-hidden="true" />
                                        </IconButton>
                                    )}
                                </div>
                            )}
